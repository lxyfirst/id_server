...
int UdpHandler::send_response(const sa_in_t& to_addr,int code,const char* message,const char* data)
{
	char buffer[256] ;

	int len = snprintf(buffer,sizeof(buffer),"{\"code\":%d,\"message\":\"%s\",\"data\":\"%s\"}",
			code , message ,data );
...
int UdpHandler::process_packet(const udp_packet* pi)
{
	...
	if(action.compare("get")==0 || action.compare("create")==0)
	{
		...
		return process_action_create(rule_name,app_name,salt,pi->addr);

	}
	else
	{
		return process_action_monitor(rule_name,app_name,pi->addr) ;
	}


}

int UdpHandler::process_action_create(const string& rule_name,const string& app_name,const string& salt,const sa_in_t& from_addr)
{
	std::string new_id ;

	if( get_app().create_id(new_id,rule_name,app_name,salt)== 0 && (new_id.size() > 1) )
	{
		send_response(from_addr,0,"success",new_id.c_str()) ;
	}
	else
	{
		send_response(from_addr,-1,"system error") ;
	}
...
int UdpHandler::process_action_monitor(const string& rule_name,const string& app_name,const sa_in_t& from_addr)
{
	Counter* counter = get_app().get_counter(rule_name,app_name) ;
	if(counter == NULL )
	{
		send_response(from_addr,-1,"not exist") ;
	}
	else
	{
		char buffer[256] = {0} ;
		int len = snprintf(buffer,sizeof(buffer),"{\"code\":0,\"message\":\"ok\",\"data\":{\"counter\":%d,\"node_offset\":%d}}",
					counter->data().counter , counter->data().node_offset);
